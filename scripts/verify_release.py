...
all_workflows=list((root/'.github/workflows').glob('*.y*ml'));mutable=hits(r'uses:\s+[^./][^@\s]+@(main|master|v?\d+(?:\.\d+){0,2})\s*$',all_workflows);ok=add('immutable_action_refs',not mutable,mutable) and ok
priv=[]
for p in all_workflows:
 if p.name in ('sign-evidence.yml','release-candidate.yml'):continue
 txt=p.read_text();
