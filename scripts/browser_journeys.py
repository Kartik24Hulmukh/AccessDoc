    ...
    with sync_playwright() as p, tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            for profile in range(20):
                ...
                locale = ["en-US", "ar", "ja-JP", "de-DE", "hi-IN"][profile // 4]
            ...
            browser.close()
    report = {"scope": "20 synthetic viewport/locale profiles x 6 journeys; no human participants",
              "checks": len(results), "passed": sum(r["status"] == "PASS" for r in results), "results": results}
    Path(args.output).write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(json.dumps({k:v for k,v in report.items() if k != "results"}))
    return int(report["passed"] != 120)
